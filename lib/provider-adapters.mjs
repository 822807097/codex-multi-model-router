...
// 官方 Responses 为服务端私有调用项分配固定 ID 前缀，跨供应商回放时第三方 Chat 转换出的
// call_…/fc_… 等 ID 不能伪造。这里用类型驱动的前缀表集中校验，新增类型只需加一行；
// 配对输出用 *_output 通配删除，覆盖任意调用类型及其未来新增的变体。
const OFFICIAL_PRIVATE_ID_PREFIXES = new Map([
  ['tool_search_call', 'tsc_'],
  ['web_search_call', 'ws_'],
  ['function_call', 'ctc'],
  ['custom_tool_call', 'ctc'],
  ['local_shell_call', 'ctc'],
]);
...
function removeForeignPrivateCallHistory(input, onDiscard) {
  ...
  const foreignCalls = new Set(input.filter((item) => {
    const prefix = OFFICIAL_PRIVATE_ID_PREFIXES.get(item?.type);
