...
export function resolveProvider(target = {}) {
  ...
  // Cursor 通道默认开启多智能体：Codex 会把 collaboration___spawn_agent 等工具透传给
  // Cursor 网关，由 Codex 桌面端当宿主执行子代理并行。早期 grok 在大量工具轮次后不写
  // 主线程文本，故默认同时注入「收尾总结」指令兜底；若某模型仍不写字，可在 target 上设
  // stripAgentTools=true 强制剥离协作工具、回到纯文字优先模式。
  const cursorChannel = /^cursor[-_]/i.test(target.name);
  const stripAgentTools = cursorChannel && target.stripAgentTools === true;
  return {
    wireApi,
    platform,
...
function removeForeignPrivateCallHistory(input, onDiscard) {
  ...
  const foreignCalls = new Set(input.filter((item) => {
    ...
    // 调用类：桌面端本地短 ID 保留，只有第三方转换的 UUID 长 ID 删除。
    if (CALL_TYPES_WITH_LOCAL_IDS.has(item?.type)) return THIRD_PARTY_UUID_ID.test(String(item.id));
    return false;
  }));
...
const ORPHAN_REPAIRABLE_CALL_TYPES = new Set(['function_call', 'custom_tool_call']);
