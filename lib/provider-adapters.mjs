...
function removeForeignToolSearchHistory(input) {
  // 官方 tool_search_call 的 id 属于服务端私有状态，第三方 Chat 转换出的 call_… 不能回放。
  // 不能伪造 ws_…：缺少官方内部状态仍会被拒绝；成对丢弃可让完整文本历史继续任务。
  const foreignCalls = new Set(input
    .filter((item) => item?.type === 'tool_search_call' && !String(item.id || '').startsWith('ws_')));
...
// 3. reasoning 输入不接受非空 content；跨模型回放时只保留摘要和加密上下文
// 4. tool_search 调用带官方私有 ws_ ID，第三方历史不能跨上游回放
...
export function adaptOfficialResponsesBody(body) {
  ...
  if (Array.isArray(body.input)) {
    for (const item of body.input) {
      if (item?.type === 'reasoning' && item.content !== undefined) delete item.content;
    }
    body.input = removeForeignToolSearchHistory(body.input);
  }
  return body;
}
