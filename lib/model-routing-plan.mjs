...
export function exposeModelRoutingState(catalog, config, configRevision, env = {}) {
  ...
  const bindings = models.map((model) => {
    // 完全锚定（$ 结尾）的精确枚举通道优先：与运行时 providerPool.candidates 的
    // 锚定优先排序一致，避免绑定预览/管理入口与实际路由不一致（2026-09-04 b.ai 实锤）。
    const ordered = [
      ...targets.map((target, index) => index).filter((index) => matchers[index] && matchers[index].source.endsWith('$')),
      ...targets.map((target, index) => index).filter((index) => !(matchers[index] && matchers[index].source.endsWith('$'))),
    ];
