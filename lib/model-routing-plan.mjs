...
import { routeMatchSafetyIssue } from '../web/route-match-safety.mjs';

const CONTROL_CHARACTERS = /[\u0000-\u001f\u007f-\u009f]/;
const MAX_NAME_LENGTH = 256;
const MAX_SAFE_TREE_DEPTH = 32;
const MAX_SAFE_TREE_NODES = 2_048;
...
function assertDedicatedTarget(target, models, operation, code) {
  const matches = typeof target.match === 'string'
    ? models.filter((model) => (
      validCatalogName(model?.slug)
      && target.match === `^${escapeModelSlug(model.slug)}$`
    ))
    : [];
  if (matches.length !== 1) {
    throw operationError(
      `${operation} 必须使用当前目录唯一模型的规范精确 match`,
      code,
    );
  }
}
  ...
  for (const operation of operationList) {
    ...
    switch (operation.kind) {
      case 'model.create': {
        ...
        models.push(clone(operation.model));
      ...
      case 'model.update': {
        ...
        models[index] = { ...models[index], ...clone(operation.patch) };
...
function inspectModelRoutes(catalog, config, errors) {
  if (!Array.isArray(catalog?.models)) return;
  const { invalid, matchers } = validTargetMatchers(config);
  // 正则错误由配置检查给出精确位置；此处不再级联成所有模型无路由。
  if (invalid) return;
  catalog.models.forEach((model, index) => {
    if (!validCatalogName(model?.slug)) return;
    if (!matchers.some((matcher) => matchesSlug(matcher, model.slug))) {
      addIssue(
        errors,
        'error',
        'model_route_missing',
...
export function inspectModelRoutingPlan({
  ...
  inspectModelRoutes(nextCatalog, nextConfig, errors);
