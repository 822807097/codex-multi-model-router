...
export function createAuthManager(options = {}) {
  const { now = () => Date.now(), defaultCooldownMs = 60000 } = options;
  ...
  function addAccount(accountData) {
    ...
    const account = {
      ...
      quota: {
        ...
        period: accountData.quota?.period || '5h',
        resetsAt: accountData.quota?.resetsAt || (now() + 5 * 3600 * 1000),
      },
  ...
  function recordQuotaUsage(id, count = 1) {
    ...
    if (currentTime > acc.quota.resetsAt) {
      acc.quota.used = 0;
      acc.quota.resetsAt = currentTime + 5 * 3600 * 1000;
    }
  ...
  function markCooldown(id, durationOrOptions = defaultCooldownMs) {
    ...
    const options = typeof durationOrOptions === 'number'
      ...
      : (durationOrOptions || {});
    acc.status = 'cooldown';
    acc.cooldownUntil = now() + (Number(options.cooldownMs) || defaultCooldownMs);
