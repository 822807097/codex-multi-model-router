...
export function createAdminHandler(options) {
  ...
  const webAssets = new Map([
    ...
    ['/admin/route-match-safety.mjs', ['route-match-safety.mjs', 'text/javascript; charset=utf-8']],
    ['/_admin/model-routing-state.mjs', ['model-routing-state.mjs', 'text/javascript; charset=utf-8']],
    ['/_admin/route-match-safety.mjs', ['route-match-safety.mjs', 'text/javascript; charset=utf-8']],
  ]);
  ...
  return async function adminHandler(req, res) {
    ...
    const isAdminRequest = url.startsWith('/_admin/') || webAssets.has(url);
