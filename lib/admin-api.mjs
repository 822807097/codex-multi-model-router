...
async function stopListenersOnPorts(ports) {
  ...
  return stopped;
    ...
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/apply-router') {
      ...
      // 勾选的 slug 必须全部存在于目录：此前垃圾/拼错的 slug 会被静默丢弃，
      // 返回「已接入（官方全集）」的假成功，用户勾选全丢。
      const knownSlugs = new Set(catalog.map((entry) => entry.slug));
      const unknownSlugs = [...new Set(requested.map((slug) => String(slug)))]
      ...
      const picked = selectModels(catalog, requested);
