...
async function stopListenersOnPorts(ports) {
  ...
  return stopped;
    ...
    if (req.method === 'GET' && url === '/_admin/api/codex-desktop/state') {
      ...
      jsonResponse(res, 200, {
        ok: true,
        mode: detectAccessMode(toml, catalog.map((e) => e.slug)),
    ...
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/restore-official') {
      ...
      const catalog = readDesktopCatalog();
      if (catalog.length === 0) {
        jsonResponse(res, 400, { error: { code: 'desktop_catalog_missing', message: '读取不到 models.json，请先确认桌面端目录存在' } });
        return true;
      }
      const official = filterOfficialModels(catalog);
      assertDesktopSafeModels(official);
      const backupToml = writeWithBackup(desktopPaths.configToml, buildOfficialConfigToml(defaultModel));
      const backupModels = writeWithBackup(desktopPaths.modelsJson, buildModelsJson(official));
      jsonResponse(res, 200, {
        ok: true,
        mode: 'official',
        models: official.length,
        backupToml,
        backupModels,
        message: `已恢复官方直连配置（${official.length} 个官方模型）；请完全退出并重启 Codex 桌面端生效`,
      });
      return true;
    }
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/apply-router') {
      ...
      const catalog = readDesktopCatalog();
      if (catalog.length === 0) {
        jsonResponse(res, 400, { error: { code: 'desktop_catalog_missing', message: '读取不到 models.json，无法接入' } });
        return true;
      }
      const requested = Array.isArray(payload?.slugs) ? payload.slugs : [];
      if (requested.length === 0) {
        jsonResponse(res, 400, { error: { code: 'desktop_models_required', message: '请至少选择一个要加载的模型' } });
        return true;
      }
      const entries = selectModels(catalog, requested);
      if (entries.length === 0) {
        jsonResponse(res, 400, { error: { code: 'desktop_models_invalid', message: '所选模型在目录中不存在' } });
        return true;
      }
      assertDesktopSafeModels(entries);
      const defaultModel = typeof payload?.defaultModel === 'string' && payload.defaultModel
        && requested.includes(payload.defaultModel)
        ? payload.defaultModel
        : entries[0].slug;
      const backupToml = writeWithBackup(desktopPaths.configToml, buildRouterConfigToml(defaultModel));
      const backupModels = writeWithBackup(desktopPaths.modelsJson, buildModelsJson(entries));
      jsonResponse(res, 200, {
        ok: true,
        mode: 'router',
        ...
        backupToml,
        backupModels,
        message: `已接入路由（${entries.length} 个模型，默认 ${defaultModel}）；请完全退出并重启 Codex 桌面端生效`,
      });
      return true;
    }
        ...
        if (req.method === 'POST' && action === 'restart') {
          try {
            ...
            jsonResponse(res, 200, {
              ...
              message: `网关已重新拉起（端口 ${gatewayPort}），约 5 秒后就绪`,
            });
            return true;
          } catch (error) {
