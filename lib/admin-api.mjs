  ...
  async function handleApi(req, res, url) {
      ...
      if (account.provider === 'openai') {
        ...
        if (rl?.fiveHour || rl?.weekly) {
          ...
          jsonResponse(res, 200, {
            ok: false,
            provider: 'openai',
            error: '暂无额度数据：经此账号发送一次请求后（Codex 额度条随响应头返回），刷新即可显示',
          });
        }
        return true;
      }
    ...
    if (req.method === 'POST' && url === '/_admin/api/accounts/set-priority') {
      ...
      if (priority === null || priority === undefined || priority === '') {
        ...
        priority = Math.max(1, Math.min(99, Math.floor(Number(priority))));
        if (!Number.isFinite(priority)) {
          jsonResponse(res, 400, { error: { code: 'invalid_priority', message: '优先级必须是 1..99 的数字' } });
          return true;
        }
      }
...
async function stopListenersOnPorts(ports) {
  ...
  return stopped;
    ...
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/restore-official') {
      ...
      // 保留桌面端主题/插件/MCP 等全部既有配置；聊天记录引用的是 models.json 里的模型，
      // 恢复官方直连绝不缩减模型目录，否则历史会话引用的模型消失会导致桌面端打不开。
      // 切回官方后桌面端会自行同步裁剪 models.json，先快照完整目录，
      // 下次「接入路由」可把被裁掉的路由侧模型自动找回。
    ...
    if (req.method === 'POST' && url === '/_admin/api/codex-desktop/apply-router') {
      ...
      // 勾选解析：当前目录 ∪ 目录快照回填。官方模式期间桌面端会同步裁剪
      // models.json，勾选中已被裁掉但快照仍保留的路由侧模型自动找回；
      // 目录与快照都没有的 slug 才是真正的未知项（此前拼错的 slug 若被
      // 静默丢弃，会返回「已接入（官方全集）」的假成功，用户勾选全丢）。
      ...
      const currentToml = readDesktopToml();
      const backupToml = writeWithBackup(
        desktopPaths.configToml,
        buildRouterConfigToml(defaultModel, currentToml, { port: options.runtime?.port ?? 15730 }),
      );
      const backupModels = writeWithBackup(desktopPaths.modelsJson, buildModelsJson(merged));
      // 接入后的目录是最新最全的状态，顺手刷新快照（此后桌面端再裁剪也有得回填）。
      snapshotDesktopCatalog(desktopPaths.modelsJson);
      jsonResponse(res, 200, {
        ok: true,
        mode: 'router',
        ...
        backupToml,
        backupModels,
        message: `已接入路由（${merged.length} 个模型，默认 ${defaultModel}）；请完全退出并重启 Codex 桌面端生效`,
      });
      return true;
    }
