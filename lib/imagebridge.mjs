 ...
 * authToken 为 Bearer 值：优先 ChatGPT 订阅账号的 OAuth access token
 * （OpenAI 接受订阅 token，账单记到 Plus/Pro 额度，对齐 sub2api：GetAccessToken → Bearer），
 * 未绑定账号时可回退环境变量平台 key。
 * @returns {Promise<{created:number, data:Array<{b64_json?:string, url?:string, revised_prompt?:string}>}>}
 */
export async function generateOpenAIImages({ authToken, payload, proxy, viaProxy = true, requestFn = rawHttpsRequest } = {}) {
  if (!authToken) {
