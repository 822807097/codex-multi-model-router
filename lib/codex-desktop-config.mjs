...
export function buildModelsJson(entries) {
  ...
  const renamed = entries.map((entry) => {
    ...
    // available_in_plans = 官方内置模型特征（推荐列表/套餐），自定义条目没有——
    // 官方内置的 GLM 保留原名（它走官方通道），只改自定义侧，避免误改名官方模型。
    if (/^glm-/i.test(slug) && !/-router$/i.test(slug) && !entry?.available_in_plans) {
      const displayName = entry.display_name
        ? `${String(entry.display_name).replace(/ \(路由\)$/, '')} (路由)`
        : `${slug} (路由)`;
      return { ...entry, slug: `${slug}-router`, display_name: displayName };
    }
    return entry;
  });
