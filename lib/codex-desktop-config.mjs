...
export function buildModelsJson(entries) {
  ...
  // when using Codex with a ChatGPT account" 400——slug 自动加 -router 后缀
  // 规避（显示名不动，选择器里加「(路由)」标识区分两个同名模型）。
  const renamed = entries.map((entry) => {
    const slug = typeof entry?.slug === 'string' ? entry.slug : '';
    // 桌面端按 slug 片段识别官方内置模型（GLM-5.3-Flash / glm-5.3 官方内置）：
    // 后缀 -router 仍会被官方片段匹配（2026-09-03 二次实锤：GLM-5.3-Flash-router
    // 依然直连官方 400）。自定义条目（无 available_in_plans）彻底改为 zhipu-
    // 前缀全新 slug，官方内置 GLM 保留原名走官方通道。
    if (/^glm-/i.test(slug) && !/^zhipu-/i.test(slug) && !entry?.available_in_plans) {
      const displayName = entry.display_name
        ? String(entry.display_name).replace(/ \(路由\)$/, '')
        : slug;
      return {
        ...entry,
        slug: `zhipu-${slug.replace(/^glm-/i, 'glm').toLowerCase()}`,
        display_name: `${displayName} (路由)`,
      };
    }
    return entry;
  });
