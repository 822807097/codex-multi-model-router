...
export function snapshotDesktopCatalog(modelsJsonPath) {
  try {
    ...
    JSON.parse(content);
    const snapshotPath = path.join(path.dirname(modelsJsonPath), CATALOG_SNAPSHOT_NAME);
...
export function readCatalogSnapshot(modelsJsonPath) {
  try {
    const data = JSON.parse(
      fs.readFileSync(path.join(path.dirname(modelsJsonPath), CATALOG_SNAPSHOT_NAME), 'utf8'),
    );
