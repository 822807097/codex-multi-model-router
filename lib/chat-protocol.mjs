...
export function responsesToChatMessages(input, options = {}) {
  ...
  if (options.autonomy !== false) {
    systems.unshift({
      role: 'system',
      content: '重要：请自主使用可用工具持续推进任务直至完成；需要信息时直接调用工具获取，不要只描述计划而不调用工具。',
    });
  }
...
function safeChatToolName(rawName, usedNames) {
  // Chat 工具名限制为 64 字符和安全字符集，冲突时追加稳定哈希。
  const normalized = String(rawName || 'tool').replace(/[^A-Za-z0-9_-]/g, '_');
  let candidate = normalized.length <= 64
    ? normalized
    : `${normalized.slice(0, 53)}_${shortHash(normalized)}`;
  if (usedNames.has(candidate) && usedNames.get(candidate) !== rawName) {
    candidate = `${candidate.slice(0, 53)}_${shortHash(rawName)}`;
  }
  usedNames.set(candidate, rawName);
  return candidate;
}
...
export function convertResponsesTools(tools, input = []) {
  ...
  const addTool = (tool, namespace = null) => {
    ...
    if (addedSourceNames.has(sourceName)) return;
    addedSourceNames.add(sourceName);
    const chatName = safeChatToolName(sourceName, usedNames);
