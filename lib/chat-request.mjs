...
function detectTrailingLoop(compactedInput) {
  const calls = [];
  for (let i = compactedInput.length - 1; i >= 0; i -= 1) {
    const item = compactedInput[i];
    if (item?.type === 'function_call' || item?.type === 'custom_tool_call') {
      calls.push(item);
      if (calls.length > LOOP_BREAKER_THRESHOLD + 6) break;
      continue;
    }
  ...
  // 与末尾调用完全相同（名称+参数哈希）且按时间倒序连续
  const first = calls[0];
  const sig = (c) => `${c.name || '?'}|${String(c.arguments ?? c.input ?? '')}`;
  const firstSig = sig(first);
  let run = 0;
  for (const c of calls) {
    if (sig(c) === firstSig) run += 1;
    else break;
  }
  return run >= LOOP_BREAKER_THRESHOLD ? { repeats: run, signature: firstSig.slice(0, 160) } : null;
}
...
async function buildChatRequest(attemptBody, target, provider, model, context) {
    ...
    if (loopHit) {
      baseRequest.messages = [
        ...
        {
          role: 'user',
          content: `[系统熔断提示] 检测到你在本次会话末尾连续重复了 ${loopHit.repeats} 次完全相同的工具调用。这是无效循环：继续重复不会产生新结果。请立即停止重复，改为：1) 总结当前已确认的状态与阻塞原因；2) 换一个不同的方法或工具达成目标；3) 如果目标依赖外部系统的确认而该系统持续未响应，明确告知用户该依赖不可用并请求人工介入。不要再执行与刚才相同的调用。`,
        },
      ];
      flog({
