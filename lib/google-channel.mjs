...
export async function openGoogleChatStream({
  ...
  if (oversizedRepeats) {
    ...
    return errorResponse(
      429,
      `${model} 已连续 ${oversizedRepeats} 次对同级别请求（本次 ${mb}MB）触发分钟级配额 429：单请求即超该模型的分钟配额桶，等待重试不会恢复。请在客户端切换其他模型（各模型配额独立），或精简上下文/新开会话后重试`,
      0,
    );
  }
  ...
  for (let attemptNo = 0; attemptNo < maxAccountAttempts; attemptNo += 1) {
    ...
    if (upstream.status === 429) {
      ...
      const oversizeHint = bodyBytesEstimate >= OVERSIZED_429_MIN_BYTES
        ...
        : '';
      noteFailure(429, `${model} 触发谷歌分钟级配额限制（已自动尝试 ${triedAccounts.size} 个账号）${oversizeHint}，请约 1 分钟后重试或临时换用其他模型`);
      continue;
    }
