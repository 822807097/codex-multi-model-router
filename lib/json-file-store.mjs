...
function revisionConflict(message = 'JSON 文件 revision 已变化') {
...
function readBoundedBytes(filePath, options = {}) {
  ...
  if (pathStat.size > maxBytes) {
    const error = issue('JSON 文件超过大小上限');
  ...
  return withOpenDescriptor(fileSystem, filePath, openReadFlags(fileSystem), (descriptor) => {
    ...
    if (finalStat.size > maxBytes || total > maxBytes) {
      const error = issue('JSON 文件超过大小上限');
      error.code = 'json_too_large';
      throw error;
    }
  ...
  const replace = () => {
    ...
    try {
      ...
      if (
        ...
        throw revisionConflict('JSON 目标文件在写入期间已更换');
      }
...
/**
 * 在 revision 未变化时提交单个 JSON 文件。
 *
 * 新内容先写入同目录临时文件并 fsync；旧文件复制到唯一备份暂存文件并校验原始
 * 字节 hash。只有上述阶段全部完成且目标 revision 仍匹配时，才原子安装 `.bak`
 * 并替换目标。提交前的任何失败都不会覆盖当前目标文件。
 */
export function commitRevisionedJson(filePath, value, expectedRevision, options = {}) {
  if (typeof expectedRevision !== 'string' || !/^[a-f0-9]{64}$/.test(expectedRevision)) {
    throw new TypeError('expectedRevision 必须是 64 位小写十六进制 revision');
  }
  const fileSystem = options.fileSystem || fs;
  const randomUUID = options.randomUUID || crypto.randomUUID;
  const backupPath = options.backupPath || `${filePath}.bak`;
  if (backupPath === filePath) throw new TypeError('backupPath 不得与 JSON 路径相同');
  ...
  const prepared = prepareJsonWrite(filePath, value, options);
  const backupStagePath = uniqueTempPath(backupPath, fileSystem, randomUUID);
  let backupStageIdentity = null;
  let backupInstalled = false;

  const cleanupBackupStage = () => {
    if (!backupStageIdentity || backupInstalled) return;
    try {
      removeOwnedTemp(fileSystem, backupStagePath, backupStageIdentity, false);
    } catch {
      // 清理失败不能掩盖提交阶段的原始错误。
    }
  };
  ...
  try {
    ...
    copyAndVerify(filePath, backupStagePath, expectedRevision, options);
    backupStageIdentity = assertRegularFile(fileSystem.lstatSync(backupStagePath));
    assertExpectedRevision();

    assertOwnedTemp(fileSystem, backupStagePath, backupStageIdentity);
    fileSystem.renameSync(backupStagePath, backupPath);
    backupInstalled = true;

    const installedBackup = readRevisionedJson(backupPath, options);
    if (installedBackup.revision !== expectedRevision) {
      throw issue('JSON 备份安装后校验失败');
    }
    // 安装备份与替换目标之间再检查一次，覆盖同 inode 的并发改写。
    assertExpectedRevision();
    prepared.replace();
    ...
    cleanupBackupStage();
    try { prepared.cleanup(); } catch { /* 不掩盖原错误 */ }
    throw error;
  }
}
