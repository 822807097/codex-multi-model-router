...
export function createRequestDiagnostics(options = {}) {
  ...
  function markFailure(fields = {}) {
    pendingFailure = {
      ...
      // 上游错误 message 摘要：<300 字符 + 去除控制字符，保留可读诊断不泄露正文。
      ...(typeof fields.error_message === 'string' && fields.error_message.trim()
        ? { error_message: fields.error_message.replace(/[\u0000-\u001f\u007f]/g, ' ').trim().slice(0, 300) }
        : {}),
  ...
  return {
    ...
    // 终态快照（供请求日志查看器等消费）：只含诊断元数据，不含请求/响应正文。
    summary() {
      const failure = pendingFailure || null;
      return {
        outcome: failure?.outcome || (terminal ? 'completed' : 'running'),
        error_code: failure?.error_code || '',
        error_message: failure?.error_message || '',
        upstream_status: typeof upstreamStatus === 'number' ? upstreamStatus : null,
        target,
        wire_api: wireApi,
        attempt,
        failover_count: failoverCount,
