...
// 完全锚定（正则 source 以 $ 结尾）= 精确枚举归属；无 $ = 前缀/宽松匹配。
function isAnchoredExact(target) {
  return target?.match instanceof RegExp && target.match.source.endsWith('$');
}
...
export class ProviderPool {
  ...
  candidates(model, affinityKeys = [], preferredTarget = null) {
    ...
    // 完全锚定（match 以 $ 结尾）的精确枚举通道优先于宽松前缀通道：厂商专属通道
    // （^(?:a|b)$，用户显式把 slug 归入该厂商）不应被先声明的宽匹配通道（^glm 等）
    // 抢走路由（2026-09-04 实锤：b.ai 枚举通道被 ^glm 智谱通道抢占）。同类内保持
    // 原声明顺序，failover 序列语义不变。
    const ranked = [
      ...matched.filter((target) => isAnchoredExact(target)),
      ...matched.filter((target) => !isAnchoredExact(target)),
    ];
