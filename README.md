...
- 内置本机 Web UI：查看通道状态，并新增、编辑、改名、删除自定义模型
...
**启动入口与管理页**：看到 `codex-router listening on 127.0.0.1:15730` 后，直接在浏览器打开 <http://127.0.0.1:15730/admin/> 即可进入本地 Web UI。它和路由只监听本机；若你在 `config.json` 中修改了端口，请把 URL 中的 `15730` 换成实际端口。健康检查入口为 <http://127.0.0.1:15730/healthz>。
...
1. **启动并打开页面**：运行 `scripts\\start-router.ps1`（后台运行则用 `scripts\\start-router-silent.vbs`），在浏览器打开 `http://127.0.0.1:15730/admin/`。页面打不开时，先打开 `/healthz`；健康检查不通说明路由尚未启动或端口不是 `15730`。
2. **先看总览和模型通道**：总览显示监听端口、运行时长、通道数量和启动告警；“模型通道”显示每个目标匹配哪些模型、协议、直连/代理方式及凭据是否已就绪。页面不会显示 API Key 或登录 Token。
3. **新增自定义模型**：打开“自定义模型”，点击“新增自定义模型”。默认选择“新建专属通道”最容易理解：填写模型 slug、显示名称和通道参数后，页面会为该模型生成精确匹配规则。已有宽匹配通道已经覆盖该 slug 时，才选择“复用已匹配通道”。
4. **编辑、改名或删除**：在同一区域选择现有模型即可编辑；改名会同时检查模型目录与通道引用，删除时会说明是否能安全删除专属通道。误操作可点击“撤销最近更改”。高级参数（代理、视觉中继、检查点和现有通道）放在“路由配置”，与自定义模型草稿不能同时保存，先保存或撤销其中一侧再切换。
5. **预检并保存**：点击“预检更改”，修正错误后点击“保存自定义模型”或“保存配置”。涉及删除或替换时，页面会展示影响并要求确认；若提示文件已变化，请点击“重新载入”，不要用旧页面强行覆盖。
6. **让配置生效**：保存成功后，等合适时机运行 `scripts\\restart-router.ps1`，然后重启 Codex 桌面端。重新打开同一个任务窗口时，官方模型和自定义模型会一起出现在模型菜单中，可继续切换使用。

> Web UI 不提供密钥输入框。第三方 API Key 仍应通过环境变量配置；修改 key 后也要重启路由。页面中的“凭据就绪”只表示对应环境变量已设置，不会读取或展示其内容。
